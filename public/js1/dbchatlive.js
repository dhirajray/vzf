  ...
  if(typeof json !='undefined'){
    ...
    //var myRegExp = /(http(s)?:\\)?([\w-]+\.)+[\w-]+[.com|.in|.org]+(\[\?%&=]*)?/g;
      ...
      var msgTemplate = '<li class="sender" '+lastModified+' '+fileName+'>\
            ...
            <img src="'+IMGPATH+'/users/small/'+userpic+'" width="32" height="32" class="userImg">\
        </li>';
...
$(function (){
    ...
    else var socket = io.connect('https://secure.onserro.com:5000',{'forceNew':true,
        'max reconnection attempts' : max_socket_reconnects,
        secure: true,'reconnect': true});

    socket.on('user logout',function(info){
        if(info.uStatus=="DELETE"){
            msg = "Your account has been deleted by the platform administrator.<br><br>You will now be logged out.";
        } else if( info.uStatus=="INACTIVE"){
            msg = "Your account has been deactivated by the platform administrator.<br><br>You will now be logged out.";
        }
        else {
            msg = "Your session is no more alive, you are being logged out.";
        }
            
        $dbConfirm({
            content: msg ,
            yesLabel:'OK',
            yes: true, yesClick:function(){gologout()},no:false
        });
    })      
    ...
    socket.on('initial notes', function(users,senderID){
        ...
        if(SESS_USER_ID==senderID)
        {
                ...
                for(var i=0; i<totuser;i++)
                {
                    ...
                    if(chatSource!='group'){    
                        allUserSelectJson.push({id:userID,text:userName,src:picture});
                         ...
                         apptr +='<li username="'+ userName.toLowerCase()+'" data-id="'+userID+'" is-accepted="'+isAccepted+'" group-owner="'+groupOwner+'"  chat-source="'+chatSource+'" data-pic="'+picture+'">\
                            <div class="chatUserPic">\
                                <span class="liveStatus '+Astatus+'"></span>\
                                <img height="32" src="'+IMGPATH+'/users/small/'+picture+'" width="32" height="32" />\
                            </div>\
                        ...
                        if(groupOwner==SESS_USER_ID){
                            ...
                            //addUsersInChatGroups('add', userID,userName);
                             ...
                             apptrOtherGroup +='<li data-id="'+userID+'" group-owner-name="'+ownerName+'" group-owner-pic="'+ownerPic+'" is-accepted="'+isAccepted+'" group-owner="'+groupOwner+'"  chat-source="'+chatSource+'" data-pic="chat-group-white.png">\
                            <div class="chatUserPic"><span class="chUserPic" style="background-image:url('+BASE_URL+'/img/'+picture+')"></span></div>\
                            <div class="chatUserDetails">\
                                <div class="oneline Uname">'+ userName+'</div>\
                            </div>\
                            '+leaveGroupCloseIcon+'\
            ...
            $.allUserChat({content:'<ul class="allUserList">'+myGroupList+apptrMyGroup+otherGroupList+apptrOtherGroup+normalUserList+apptr+'</ul>', class:noUserFound, allUser:true});
             var getUserName = [];
            $('.allUserList li[username]').each(function (){
                 getUserName.push($(this).attr('username'));
            });
            //console.log(getUserName);
           // console.log(getUserName.sort());
           var alphabeticalUsers = getUserName.sort().reverse();
            $.each(alphabeticalUsers, function (i, v){
                var userGet = $('.allUserList li[username="'+v+'"]').detach();
                 $('.allUserList .itsAllUsers').after(userGet);
            });
       ...
       //console.log(chatval.fileJson)
        ...
        if(chatval.recieverID==SESS_USER_ID || chatval.senderID==SESS_USER_ID)
        {
           ...
           if(typeof chatval.noInsert!='undefined'){
            ...
            var userpicsUrl = IMGPATH+'/users/small/'+img+'';  
            var notiText  = '';  
              ...
              var fileSizeMB= chatval.fileJson.fileSizeMB;
               ...
               if(fileStatus=='success')
               {
                ...
                if(fileStatus=='cancelFile'){
                    ...
                    $('[last-modified="'+lastModified2+'"]').closest('.chatMessageContainer').perfectScrollbar('destroy');
                    $('[last-modified="'+lastModified2+'"]').closest('.chatMessageContainer').perfectScrollbar({ wheelSpeed: 100,includePadding:true });
            ...
            chatResAppend = '<li class="'+SenderType+'"  '+lastModified+' '+fileNameOnLi+' >\
                ...
                <img src="'+IMGPATH+'/users/small/'+img+'" width="32" height="32" class="userImg">\
            </li>';
           ...
           if(typeof chatval.noInsert!='undefined'){
            ...
            if(joinorleftstatus==1)
            {
                chatResAppend = '<li class="joinorleftstatus">\
                    <div class="msgAsNoti">\
                        <img src="'+IMGPATH+'/users/small/'+img+'" width="16" height="16" >\
                ...
                for(var i=0; i<totuser;i++)
                {
                   ...
                   if(joinorleftstatus==1)
                   {
                        mychats +='<li class="joinorleftstatus">\
                            <div class="msgAsNoti">\
                                 <img src="'+IMGPATH+'/users/small/'+img+'" width="32" height="32" >\
            ...
            else
            {
                ...
                for(var i=0; i<totuser;i++)
                {
                   ...
                   if(joinorleftstatus==1)
                   {
                        mychats ='<li class="joinorleftstatus">\
                            <div class="msgAsNoti">\
                                <img src="'+IMGPATH+'/users/small/'+img+'" height="32"  height="32" >\
        ...
        if(chatval.clientID==clientID)
        {
            ...
            if(calling=='chatstatus')
            {
                if(chatval.olStatus==1)
                {
                    if(isOpen>0)
                    {
                        $ghostMessage( '<img height="32"  src="'+IMGPATH+'/users/small/'+chatval.ProfilePic+'" style="float:left; margin-right:10px;"><strong>'+ chatval.full_name+'</strong><br>is now online');
                    }
                ...
                { 
                    if(isOpen>0)
                    {
                        $ghostMessage( '<img height="32" src="'+IMGPATH+'/users/small/'+chatval.ProfilePic+'" style="float:left; margin-right:10px;"> <strong>'+ chatval.full_name+'</strong><br>went offline');
                    }
    ...
    $('body').on('click','.dbchatWrapper.allUserChat .chatTittleBar li a',function(e){
       ...
       // if(CHAT_STATUS==Stval) return false;
        ...
        function _chatSelectUserFormating(data) {
                     ...
                     return '<img src="'+IMGPATH+'/users/small/'+data.src+'" width="16" height="16" />' + data.text;
...
$.chatWindows = function (options){
    ...
    var oflSel = 'online';
    var showOnlineBtn = '<li><a href="javascript:void(0);" data-value="0"><span class="liveStatus '+oflSel+'"></span>Show me offline</a></li>';
    if(CHAT_STATUS==0) {
      oflSel = "offline";
      showOnlineBtn = '<li><a href="javascript:void(0);" data-value="1"><span class="liveStatus"></span>Show me online</a></li>';
    }
   // else if(CHAT_STATUS==1) oflSel = "online";
    ...
    var defaults = {
        ...
        userName:'<div class="dropDown">\
                    <div class="myPicForChat '+oflSel+'"><img src="'+IMGPATH+'/users/small/'+userpic+'" width="35" height="35"></div>\
    ...
    if(userType=='alluser'){
        ...
        openShareBox='<div class="chatAudioRing"><audio autostart="false" class="chatRing" id="chatRing_'+SESS_USER_ID+'" controls></div>\
                          <source src="'+BASE_URL+'/img/dbchat.wav" type="audio/wav">\
    ...
    if(userType!='alluser'){  
            ...
            $.cookie('openedChildWindow-'+userType+'-'+SESS_USER_ID, userType ,{path:'/'});
             if(isMobile==null){
                ...
                $('.dbchatWrapper[chat-type="'+userType+'"] textarea').focus();
                   ...
                   $('.dbchatWrapper[chat-type="'+userType+'"] .chatMessageContainer').perfectScrollbar('destroy');
                }
         ...
         if(isMobile==null){
            $('.dbchatWrapper[chat-type="'+userType+'"] .chatContainer').perfectScrollbar({ wheelSpeed: 100,includePadding:true });
               ...
               $('.dbchatWrapper[chat-type="'+userType+'"] .chatContainer').perfectScrollbar('destroy');
            }
...
$.allUserChat = function(options){        
    ...
    if(settings.class!=''){
     ...
     if(settings.allUser==true){
        ...
        chatStartNow ='<a href="javascript:void(0);" class="chatStartNow"><i class="fa fa-comment"></i></a>';
          $('.chatStartNow').remove();
...
$('body').on('click', '.dbchatWrapper[chat-type="alluser"] .chatContainer  li:not(.itsSeprator)',function (){
            ...
            var userPicSrc =  IMGPATH+'/users/small/'+userPicName //$('.chatUserPic img',tsl).attr('src');
            ...
            if(isAccepted==0 && isAccepted!='') {
             ...
             if(chatSource=='group'){
                userPicSrc = BASE_URL+'/img/'+userPicName;
                onelineStatusHtml = '';
                ...
                var fileSharingChat = '<form id="fileSharingChat_'+dataId+'" class="dropzone">\
                 ...
                 if(isAccepted==0 && isAccepted!=''){
                    ...
                    groupChatNoficationHtml = '<div class="groupChatNoficationBox">\
                                                    ...
                                                    <div class="groupNameDes"><div class="grpOwnerInfo"><img src="'+IMGPATH+'/users/small/'+grpOwnerPic+'" width="42" height="42"><br>'+grpOwnerName+'</div> <i>has created a chat Group</i></div>\
                                               </div>';
                }
...
socket.on('my group members',function(members,grpDtls){
    ...
    $.each(allUserSelectJson, function(i, value){
        ...
        var userAct = '<a href="javascript:void(0);" class="addUserToThisGroup adrUsBtn">Add</a>';
         ...
         $.each(members, function(index, val){
            ...
            chatUserList +='<li data-id="'+value.id+'"><img src="'+IMGPATH+'/users/small/'+value.src+'" width="32" height="32"><div class="adre_userList"><span class="userNameAdre">'+value.text+'</span>'+userAct+'</div></li>';
    ...
    $('.childUList').perfectScrollbar({ wheelSpeed: 100,includePadding:true });
    setTimeout(function(){
...
$.winResize = function (){
    ...
    $('.dbchatWrapper').each(function (index){
        ...
        $('.allUserChat .chatContainer').perfectScrollbar('destroy');
    }   
}
