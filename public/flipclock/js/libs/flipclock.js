...
(function($) {
	...
	FlipClock.List = FlipClock.Base.extend({
		...
		select: function(digit) {
			...
			var targetP = this.$obj.find('[data-digit="'+digit+'"]').closest('ul.flip');
			...
			targetP.attr('mydigit', digit);
			
			if( target.closest('[mydigit="0"]:nth-child(2)').next('[mydigit="0"]')){
				setTimeout(function (){
					target.closest('[mydigit="0"]:nth-child(2)').next('[mydigit="0"]').prev().prev('.flip-clock-divider:first').remove();
					var bn = target.closest('[mydigit="0"]:first-child').next('[mydigit="0"]');
					bn.prev().prev('.flip-clock-divider').remove();
					bn.next().find('.flip-clock-dot').remove();			
					bn.prev('[mydigit="0"]').remove();
					bn.remove();
				},1000);
