...
class Application_Model_usergeoinfo
{
    ...
    public function index()
	{
		...
		foreach($infoofuser as $key=>$value){
			...
			//echo '<br>'.$value['Name'].' - '.$i++;
			
			//echo '<br>'.$value['LastLoginIP'].' - '.$value['IP'];
		    //   echo'<pre>';//print_r($value);
			   
			   
			  	$last  	=	empty($value['LastLoginIP'])?$value['IP'] :$value['LastLoginIP'] ;
			   
			 	$first 	 =	empty($value['IP'])?$value['LastLoginIP'] :$value['IP'] ;
				
				if(!empty($first) || !empty($last) )
				{
					$UserID = $value['UserID'];
					$geoplugin1 = new geoPlugin();
					$geoplugin1->locate($last);  // Based on last login IP
					
					$geoplugin = new geoPlugin();
					$geoplugin->locate($first);  // Based on registered login IP
				
				
					$users1['City'] 			 = $geoplugin1->city;
					$users1['region'] 		 = $geoplugin1->region;	
					$users1['area_code'] 	 = $geoplugin1->areaCode;
					$users1['dma'] 			 = $geoplugin1->dmaCode;
					$users1['country_code'] 	 = $geoplugin1->countryCode;
					$users1['country_name'] 	 = $geoplugin1->countryName;
					$users1['continent_name'] = $this->getcontinent($geoplugin1->continentCode);
					$users1['longitude'] 	 = $geoplugin1->longitude;
					$users1['latitude'] 		 = $geoplugin1->latitude;
					$users1['currency_code']  = $geoplugin1->currencyCode;
					$users1['currency_symbol'] = $geoplugin1->currencySymbol;
