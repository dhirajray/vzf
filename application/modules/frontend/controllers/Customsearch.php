...

	{
		...
		if ($response->hasResults()) {
		    ...
		    foreach($response->getResults() as $result) {
		        ...
		        $return.='<a href="'.$result->getLink().'" target="_blank"><div class="googleresult-title">'.$result->getTitle() . '</div></a><div class="googleresult-link">' . $result->getLink() . '</div><div><div style="float:left; width:410px">' . $result->getSnippet() . '</div><div class="button-yellow" style="margin:0 0 0 5px; float:left;" onclick="javascript:startdbfromlink(\''.$result->getLink().'\')">add link</div></div><div class="next-line"></div><div style="width:10px; height:20px;"></div>';
